...
// ====== 工具函数：生成唯一交易号 ======
function getTradeNo(): string {
  return Date.now().toString(); // 简单唯一值
}

// ====== 创建支付订单接口 ======
const createOrder = async (req: Request, res: Response) => {
  const { totalAmount, subject, body } = req.body as {
    totalAmount: string;
    subject: string;
    body?: string;
  };
  ...
  try{
    ...
    req.body.data.forEach((item:Record<string,any>) => {
...
const refund = async (req: Request, res: Response) => {
  const { out_trade_no, refund_amount } = req.body as {
    out_trade_no: string;
...
// ====== 退款查询接口 ======
const queryFund = async (req: Request, res: Response) => {
  const { out_trade_no } = req.params;
...
const closeOrder = async (req: Request, res: Response) => {
  const { out_trade_no } = req.body as { out_trade_no: string };
...
// ====== 异步回调接口（notifyUrl） ======
const notifyUrl = async (req: Request, res: Response) => {
